...
void OrderHandler::on_fair_price(const std::string& product, const FairPriceData& data) {
    ...
    // forward_vol_bp reaches the strategy's spread rather than just its logs, so a
    ...
    bool vol_usable = std::isfinite(data.forward_vol_bp) && data.forward_vol_bp > 0.0;
    it->second.fair_price = FairPriceState{
        .ts = data.ts,
        .fair_price = data.fair_price.value_or(NAN),
        .factor = data.factor.value_or(NAN),
        .forward_vol_bp = vol_usable ? data.forward_vol_bp : 1.0
    };
}
...
void OrderHandler::build_price_info(const ProductState& state, PriceInfo& price_info) {
    ...
    if (usable && std::isfinite(state.fair_price.factor)) {
        ...
        price_info.forward_vol_bp = state.fair_price.forward_vol_bp;
    } else {
